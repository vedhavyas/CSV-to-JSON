...
def csv_to_sqldb(sqldb, infilename, table_name):
	...
	try:
		...
		logging.debug("New Table Created %s;" % table_name)	
				
#        print "create table %s (%s);" % (table_name, colstr)
...
def qsqldb( sqldb, sql_cmd, sample_interval):
	...
	try:
		...
		logging.debug(sys.exc_type)
                logging.debug(sys.exc_value)
                logging.debug(sys.exc_traceback)
	...
	for i in range (num_col+1):
	  temp['column_'+str(i)]=0
        ...
        for i in range (num_row):
           datarows[i] = list(datarows[i])
           for k in range (num_col):
              if (type(datarows[i][k]) == unicode):
                     datarows[i][k] = datarows[i][k].replace('\x00','') 
           datarows[i] = tuple(datarows[i])
...
if __name__=='__main__':
	...
	logging.basicConfig(filename='querycsv.log',level=logging.DEBUG)
        # Reading the configuration file
        config = ConfigParser.ConfigParser()
        config.read('/var/www/html/sysstat/CONF.ini')
	logging.debug("Reading conf from /var/www/html/sysstat/CONF.ini")
	...
	arr2 = {'desc':0, 'cpuinfo':0, 'dmidecodereport':0, 'meminfo':0, 'sysctl':0}
        ...
        for i in range (1,chart_count+1): 
           ...
           query_string = query_string + " FROM " + ",".join(table_names)
           if (table_no >= 2):
              query_string = query_string + " WHERE "
              for j in range (1, table_no):
                 if (j != 1):
                   query_string = query_string + " AND "
                 query_string = query_string + table_names[j-1]+".ID = "+ table_names[j]+".ID"
           query_string = query_string + ';'
#           print query_string;  
           outfile="/var/www/html/sysstat/Graphdata.json"
	   hardwareoutfile = "/var/www/html/sysstat/hardwareSpec.json"
           file_db = None
           keep_db = False
           count=0
           # Adding the ".csv" to each tablename
           while (count < table_no):
              table_names[count] = "/var/www/html/sysstat/"+table_names[count] + ".csv"
              count = count + 1
...
#	   print table_names
	...
	file.close
        ...
        desc=config.get('DB', 'description')
